...
<body>
<h1><a id="id.toc" name="id.toc">Contents</a></h1><dl><dd>
...
<a id="id.s1" name="id.s1">1</a> Attachments<a href="#id.toc"><img src="apt_toc.gif" style="border-style: none; border-width: 0" width="30" height="11" alt="Contents"></a>
</h1><h2>
...
how does attachments work?</h3><p>Before you can use the attachments package, each instance of
your package must be mapped to a <tt>file-storage root
folder</tt>.</p><p>The root folder is a "super folder" for all the files in that
specific instance of file-storage, and it is created automatically
when the file-storage package is instanciated.</p><p>When a user wants to make an attachment to an object in your
...
to upload a new file into file-storage to attach.</p><h2>
<a id="id.s1.2" name="id.s1.2">1.2</a> Using attachments</h2><h3>
<a id="id.s1.2.1" name="id.s1.2.1">1.2.1</a> Mount attachments
under your package</h3><p>The first step is to mount the attachments package <b>under</b>
your package. In the <tt>site-map</tt>, you should create a new
site-node (sub-folder) under your package called "attach".</p><p>"attach" is the standard URL, however URL this can be changed on
...
instance is just re-mounted)</p><h3>
<a id="id.s1.2.2" name="id.s1.2.2">1.2.2</a> Map an
file-storage root folder to your package instance</h3><p>First, you must select a file-storage instance that will provide
...
must create one using the site-map.</p><p>Once you have a file-storage instance you wish to use, you must
map the root folder of that file-storage instance with the
package_id of the instance of your package.</p><p>Two utility procs to help you do this:</p><p>
...
package check if attachments is mounted properly</h4><p>Most likely, you want your package to work even if the
...
package with it:</p><div class="box" style="border: 2px solid; width: 100%"><pre>
...
attachment Url</h4><p>When you want to set up the link that the user can click on to
attach something to an object, use the
attachments::add_attachment_url proc, which will return the correct
Url into the attachments package mounted under your package.</p><div class="box" style="border: 2px solid; width: 100%"><pre>
...
</pre></div><p>forums redirects the user to the redirect_url, if the user chose
